...
class BlindsCover(CoverEntity, RestoreEntity):
    ...
    def __init__(self, hass: HomeAssistant, entry: ConfigEntry):
        ...
        # Tracks which phase we are in: "tilt", "tilt_only", "cover", or None
    ...
    # ------------------------------------------------------------------
    # Cover commands — open/close/stop/set_position
    # Sequence: tilt first (fully open or closed), then cover travels
    # ------------------------------------------------------------------

    async def async_open_cover(self, **kwargs):
        """Open the cover — tilt fully open first, then raise cover."""
    ...
    async def async_close_cover(self, **kwargs):
        """Close the cover — tilt fully closed first, then lower cover."""
        self._movement_generation += 1
        if not self.has_tilt_support():
        ...
        if self.tilt_calc.current_position() > 0:
            ...
            # Tilt already closed, lower cover directly
            self._movement_phase = "cover"
            self.travel_calc.start_travel_down()
            self.start_auto_updater(COVER_UPDATE_INTERVAL)
            await self._async_handle_command(SERVICE_CLOSE_COVER)
    ...
    # ------------------------------------------------------------------
    # Auto updater — drives the tilt → cover phase state machine
    # ------------------------------------------------------------------

    def start_auto_updater(self, interval_seconds=COVER_UPDATE_INTERVAL):
        """Start the auto updater with the given interval."""
    ...
    def restart_auto_updater(self, interval_seconds):
        """Restart the auto updater with a new interval."""
        self.stop_auto_updater()
        self.start_auto_updater(interval_seconds)
    ...
    def auto_updater_hook(self, now: datetime) -> None:
        ...
        if self._movement_phase is None:
            return  # stale tick after stop, ignore it
        ...
        if self._movement_phase in ("tilt", "tilt_only"):
            if self.tilt_calc.position_reached():
                ...
                if self._movement_phase == "tilt_only":
                    ...
                    # Tilt phase done — transition to cover phase with slower interval
                    self._movement_phase = "cover"
                    target = self._pending_cover_target
                    self._pending_cover_target = None
                    self.restart_auto_updater(COVER_UPDATE_INTERVAL)
                    generation = self._movement_generation
                    self.hass.async_create_task(self._start_cover_phase(target, generation))
    ...
    async def _start_cover_phase(self, target: int, generation: int) -> None:
        """Start cover movement after tilt phase completes."""
        if generation != self._movement_generation:
            return  # stale task — movement already ended or changed

        current = self.travel_calc.current_position()
        if current == target:
            # Already at target, just stop
            await self._async_handle_command(SERVICE_STOP_COVER)
            return
    ...
    async def auto_stop_if_necessary(self):
        ...
        if self._send_stop_at_end:
            self._movement_generation += 1  # invalidate any pending tasks
    ...
    # ------------------------------------------------------------------
    # State restore & known position services
    # ------------------------------------------------------------------
